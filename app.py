...
def build_prompt(exam_type, grade, subject, count, lang="en", task="generate", answers=None, student_id=""):
    ...
    if task == "generate":
        ...
        if lang == "ar":
            ...
            if "بيرلز" in exam_type_lower or "pirls" in exam_type_lower:
                framework_context = f"""
            اختبار PIRLS هو اختبار دولي يقيس مهارات الفهم القرائي لدى طلاب الصف {grade}.
            الهدف هو تقييم قدرة الطالب على القراءة والفهم من خلال نصوص قصيرة متنوعة ومناسبة للعمر.

            📘 مكونات الاختبار:
            - يتكوّن الامتحان من {count} أسئلة.
            - كل سؤال يحتوي على **نص واقعي قصير (يتراوح بين 80 إلى 120 كلمة)** يعرض موقفًا حقيقيًا بسيطًا أو قصة قصيرة جدًا.
            - بعد كل نص، يوجد **سؤال واحد فقط** مع **أربعة خيارات** تستند جميعها إلى النص نفسه.
            - لا يجوز أن يُكتب سؤال بدون نص قبله.

            🧠 تعليمات إنشاء النصوص والأسئلة:
            - يجب أن تكون النصوص متنوعة (اجتماعية، طبيعية، مدرسية، حياتية...) لكنها واقعية وواضحة.
            - لا تستخدم عناصر خيالية أو رموز أو حيوانات متحدثة.
            - اجعل اللغة بسيطة ومناسبة لعمر الصف {grade}.
            - تأكد أن السؤال يستند مباشرة إلى النص المكتوب وليس معرفة خارجية.
            - اجعل الخيارات منطقية ومبنية على تفاصيل النص.
            - نوع المهارات بين (الفكرة الرئيسة، التفاصيل، المعنى، الاستنتاج، الغرض).
            ...
            "questions": [
                ...
                "question": "اكتب سؤالًا يعتمد مباشرة على النص السابق.",
                "options": ["الخيار أ", "الخيار ب", "الخيار ج", "الخيار د"],
                "correct_answer": "الخيار الصحيح فقط",
                "skill": "حدد المهارة (مثلاً: الفكرة الرئيسة أو التفاصيل)"
                }},
                {{
                "id": 2,
                "reading_passage": "اكتب نصًا جديدًا واقعيًا آخر (80–120 كلمة).",
                "question": "سؤال يعتمد على النص الثاني.",
                "options": ["...", "...", "...", "..."],
                "correct_answer": "...",
                "skill": "المعاني أو الاستنتاج"
                }}
            ]
            }}

            ✳️ ملاحظات مهمة:
            - يجب أن يحتوي كل سؤال على نصه الخاص.
            - لا تكرر نفس الكلمات أو الفكرة بين الأسئلة.
            - لا تكتب أي شيء خارج صيغة JSON.
            """
