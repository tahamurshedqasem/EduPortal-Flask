...
def build_prompt(exam_type, grade, subject, count, lang="en", task="generate", answers=None, student_id=""):
    ...
    if task == "generate":
        ...
        if lang == "ar":
            ...
            الهدف هو قياس قدرة الطالب على فهم نص واقعي قصير والإجابة على سؤال يعتمد عليه.

            📘 مكونات الاختبار:
            - يتكون الامتحان من {count} أسئلة.
            - في كل سؤال، يجب أن تكتب **نصًا واقعيًا قصيرًا (80–120 كلمة)** مناسبًا لعمر طلاب الصف {grade}.
            - بعد النص مباشرة، اكتب **سؤالًا واحدًا فقط** يعتمد على تفاصيل النص نفسه.
            - لا تستخدم أي حقل منفصل للنص. ضع النص والسؤال معًا في الحقل "question".
            - يجب أن تكون جميع الخيارات الأربعة منطقية ومبنية على النص.
            - يمنع تمامًا إدراج أسئلة بدون نص واضح.
            ...
            "questions": [
                ...
                "id": 1,
                "difficulty": "سهل",
                "question": "اكتب هنا نصًا واقعيًا قصيرًا (80–120 كلمة) مناسبًا للصف {grade}، ثم في السطر التالي اطرح سؤالًا يعتمد على النص فقط.",
                "options": ["الخيار أ", "الخيار ب", "الخيار ج", "الخيار د"],
                "correct_answer": "الخيار الصحيح",
                "skill": "الفكرة الرئيسة أو التفاصيل"
                }},
                {{
                "id": 2,
                "difficulty": "متوسط",
                "question": "اكتب هنا نصًا واقعيًا آخر مختلفًا ثم سؤالًا يعتمد على النص نفسه.",
                "options": ["...", "...", "...", "..."],
                "correct_answer": "...",
                "skill": "الاستنتاج أو المعنى"
                }}
            ]
            }}

            ✳️ ملاحظات مهمة:
            - يجب أن يحتوي كل عنصر على فقرة قصيرة تليها مباشرة جملة السؤال داخل نفس الحقل "question".
            - لا تكتب حقل باسم "reading_passage".
            - أرجع فقط JSON صالح بدون أي شرح خارجي.
            """
