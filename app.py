...
def build_prompt(exam_type, grade, subject, count, lang="en", task="generate", answers=None, student_id=""):
    ...
    if task == "generate":
        ...
        if lang == "ar":
            ...
            if "بيرلز" in exam_type_lower or "pirls" in exam_type_lower:
                framework_context = f"""
            اختبار PIRLS هو اختبار دولي يقيس الفهم القرائي لدى طلاب الصف {grade}.
            الهدف هو تقييم قدرة الطالب على القراءة والفهم من خلال نصوص قصيرة متنوعة.

            📘 مكونات الاختبار:
            - يتكون كل سؤال من **نص واقعي قصير واحد** (يتراوح بين 80 إلى 120 كلمة) مناسب لعمر طلاب الصف {grade}.
            - بعد كل نص، اطرح **سؤالًا واحدًا فقط** يعتمد كليًا على ذلك النص.
            - يجب أن تكون **الخيارات الأربعة** في السؤال مبنية على محتوى النص نفسه، وليست عامة أو تخيلية.
            - يمكن أن تكون النصوص معلوماتية أو قصصية بسيطة، لكن يجب أن تكون واقعية وواضحة.

            🔹 مهم جدًا:
            - لا تبتكر أحداثًا أو أسماء خيالية.
            - لا تطرح أسئلة عامة خارج النص.
            - يجب أن يُكتب النص أولًا، ثم السؤال، ثم الخيارات.

            🧠 المهارات التي يجب قياسها في الأسئلة:
            - الفكرة الرئيسة
            - التفاصيل الداعمة
            - المفردات والمعاني
            ...
            "questions": [
                ...
                "question": "اكتب هنا سؤالًا يعتمد مباشرة على النص السابق.",
                "options": ["...", "...", "...", "..."],
                "correct_answer": "...",
                "skill": "حدد المهارة (مثلاً: الفكرة الرئيسة، التفاصيل، الاستنتاج)"
                }},
                {{
                "id": 2,
                "reading_passage": "نص واقعي جديد مختلف عن الأول (80–120 كلمة).",
            ...
            أرجع فقط كائن JSON صحيح يحتوي على قائمة من الأسئلة، بحيث يحتوي كل سؤال على نص القراءة الخاص به وسؤاله وخياراته.
